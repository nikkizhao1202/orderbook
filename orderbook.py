...
def new_book():
    buy_orders.clear() #The clear() method removes all items from the dictionary
    sell_orders.clear()
...
def new_order( order_side, order_price, order_qty, display_book = False ):
    ...
    if ( order_side == 's' ):
        ...
        if len( buy_orders ) > 0:
            ...
            for key in sorted( buy_orders, reverse = True ): #large first 
                buy_price = key
    ...
    # it's a buy side
        ...
        if len( sell_orders ) > 0:
            ...
            for key in sorted( sell_orders ): #smallest first 
                sell_price = key
